...
int invoke_ocl_kernel(IplImage *in_img, IplImage *out_img, const char *filename, const char *kernel_name, cl_device_type device_type)
{
	cl_int status = 0;
...
{
	cl_int status = invoke_ocl_kernel(in_img,out_img,BOX3X3_KERNEL_FILENAME, "vx_box3x3", CL_DEVICE_TYPE_GPU);
	if(status==CL_SUCCESS)
	{
		status = invoke_ocl_kernel(out_img,out_img,NOT_KERNEL_FILENAME, "vx_not", CL_DEVICE_TYPE_GPU);
	}
	return status;
}
