...
AppAddSub::AppAddSub(Context &context, vx_kernel_e kernel_e)
...
void AppAddSub::setup()
{
	...
	spolicy = vxCreateScalar(mContext.getVxContext(), VX_TYPE_ENUM, &policy);
}

void AppAddSub::process(int variant_numer)
{
	enum Target targets[APP_ONE_NODE];
	getVariantTarget(variant_numer, targets);
	Node *node = mGraph->addNode(mKernel_es[0], targets[0]);
	node->connect(4, in1->getVxImage(), in2->getVxImage(), spolicy, out->getVxImage());
	if (mGraph->verify())
		mGraph->process();
...
void AppAddSub::profiling(int n_times, int variant_numer)
{
	enum Target targets[APP_ONE_NODE];
	getVariantTarget(variant_numer, targets);
	Node *node = mGraph->addNode(mKernel_es[0], targets[0]);
...
void AppAddSub::releaseInput()
{
	src1.release();
	src2.release();
}

void AppAddSub::generateApps(Context &context, std::vector<Application *> *apps)
{
	vx_kernel_e kernels[] = { VX_KERNEL_ADD, VX_KERNEL_SUBTRACT };
	...
	{
		AppAddSub *app = new AppAddSub(context, kernels[i]);
#if defined(EXPERIMENTAL_USE_HEXAGON) && defined(EXPERIMENTAL_USE_OPENCL) && defined(EXPERIMENTAL_USE_FASTCV)
