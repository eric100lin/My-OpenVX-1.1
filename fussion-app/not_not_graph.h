...
vx_status not_not_graph(vx_context context, Mat inMat, Mat &outMat)
{
	...
	vx_kernel not_kernel_2 = vxGetKernelByName(context, VX_CL_BOX3X3_NAME);
	CHECK_NOT_NULL(not_kernel_2, "OpenVX box3x3 kernel");
	
	//Node
	vx_node not_node = vxCreateGenericNode(graph, not_kernel);
	CHECK_NOT_NULL(not_node, "OpenVX not node");
	vx_node not_node_2 = vxCreateGenericNode(graph, not_kernel_2);
	...
	status = vxQueryNode(not_node_2, VX_NODE_ATTRIBUTE_PERFORMANCE, &perf_box3x3, sizeof(vx_perf_t));
	...
	status = vxReleaseKernel(&not_kernel_2);
	CHECK_STATUS(status, "vxReleaseKernel(&not_kernel_2)");
