 ...
 * \brief The FastCV OpenVX Kernel Interfaces
...
static const vx_char name[VX_MAX_TARGET_NAME] = "android.fastcv";
...
vx_status vxTargetInit(vx_target target)
{
    ...
    if (target)
    {
        ...
        target->priority = VX_TARGET_PRIORITY_FASTCV;
...
{
	vx_status status_hex = vxFastCVDeInit();
